...
def convert_model(timestamp: str) -> None:
    ...
    onnx_model = onnx.load("/tmp/detector.onnx")
    tf_rep = prepare(onnx_model)
    tf_rep.export_graph("/tmp/output_path")  # export the model

    modeltf = tf.saved_model.load("/tmp/output_path")
    ...
    pathlib.Path("/tmp/anchors.json").write_text(json.dumps(anchors))
